...
bool IsColision(const Segment& segment, const Plane& plane) {
	...
	float a = plane.distance - Dot(segment.origin, plane.normal);
	float t = a / dot;
	/*if (t >= 0 && t <= 1) {
		return true;
	}
...
// 衝突判定
bool IsColision(const Triangle& triangle,const Segment& segment) {
	Vector3 v1 = Subtract(triangle.vertice[1], triangle.vertice[0]);
	Vector3 v2 = Subtract(triangle.vertice[2], triangle.vertice[1]);

	Plane plane;
	plane.normal = Normalize(Cross(v1, v2));

	plane.distance = Dot(triangle.vertice[0], plane.normal);
	...
	if (IsColision(segment, plane)) {
		...
		// 衝突点
		Vector3 p = Add(segment.origin, Project(segment.origin, Multiply(t,segment.diff)));

		// 各辺を結んだベクトル
		Vector3 v01 = Subtract(triangle.vertice[1], triangle.vertice[0]);
		Vector3 v12 = Subtract(triangle.vertice[2], triangle.vertice[1]);
		Vector3 v20 = Subtract(triangle.vertice[0], triangle.vertice[2]);
		// 頂点と衝突点ｐを結んだベクトル
		Vector3 v1p = Subtract(p, triangle.vertice[1]);
		Vector3 v2p = Subtract(p, triangle.vertice[2]);
		Vector3 v0p = Subtract(p, triangle.vertice[0]);
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Segment segment = { {-0.5f,0.5f,0.0f},{1.0f,0.5f,0.0f} };
	Triangle triangle;
	triangle.vertice[0] = { -1.0f,0.0f,0.0f };
	triangle.vertice[1] = { 0.0f,1.0f,0.0f };
	triangle.vertice[2] = { 1.0f,0.0f,0.0f };
	int color = WHITE;
	...
	while (Novice::ProcessMessage() == 0) {
		...
		for (uint32_t i = 0; i < 3; ++i) {
			Vector3 ndcVertex = Transform(triangle.vertice[i], worldViewProjectionMatrix);
			screenVertices[i] = Transform(ndcVertex, viewportMatrix);
		}

		if (IsColision(triangle,segment)) {
			color = RED;
		}
		else {
			color = WHITE;
		...
		ImGui::DragFloat("triangle.vertice[0]", &triangle.vertice[0].x, 0.01f);
		ImGui::DragFloat("triangle.vertice[1]", &triangle.vertice[1].x, 0.01f);
		ImGui::DragFloat("triangle.vertice[2]", &triangle.vertice[2].x, 0.01f);
		...
		Novice::DrawLine(int(screenVertices[1].x), int(screenVertices[1].y), int(screenVertices[2].x), int(screenVertices[2].y), WHITE);
		Novice::DrawLine(int(screenVertices[2].x), int(screenVertices[2].y), int(screenVertices[0].x), int(screenVertices[0].y), WHITE);
		///
		/// ↑描画処理ここまで
		///
